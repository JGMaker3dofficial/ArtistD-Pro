...
void spiFlashErase_PIC()
{
	...
	#else
	//erase 0x003000 -64K
	for(pic_sectorcnt=0;pic_sectorcnt<(64 - 12)/4;pic_sectorcnt++)
	{
		W25QXX.SPI_FLASH_SectorErase(PIC_NAME_ADDR+pic_sectorcnt*4*1024);
	}
	//erase 64K -- 6M
	for(pic_sectorcnt=0;pic_sectorcnt<95;pic_sectorcnt++)
	{
		W25QXX.SPI_FLASH_BlockErase(PICINFOADDR+(pic_sectorcnt+1)*64*1024);
	}
	#endif
	/*	
	FLASH_Unlock();

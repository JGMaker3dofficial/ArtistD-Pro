	...
	switch(obj->mks_obj_id)
	{
		case ID_F_ADD:
			if(event == LV_EVENT_CLICKED) {
				...
				if(thermalManager.fan_speed[0]+1 <= 255)
				{	
...
void lv_draw_fan(void)
{
	...
	lv_obj_t *buttonHigh = lv_imgbtn_create(scr, NULL);
	lv_obj_t *buttonMid = lv_imgbtn_create(scr, NULL);
	lv_obj_t *buttonOff = lv_imgbtn_create(scr, NULL);

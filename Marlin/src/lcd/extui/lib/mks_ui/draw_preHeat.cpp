...
void disp_step_heat(void)
{
	...
	else if(uiCfg.stepHeat == 50)
		lv_obj_set_event_cb_mks(buttonStep, event_handler, ID_P_STEP, "bmp_Step50_degree.bin",0);
	...
	if(gCfgItems.multiple_language != 0)
	{
		...
		{
			lv_label_set_text(labelStep, preheat_menu.step_50c);
			lv_obj_align(labelStep, buttonStep, LV_ALIGN_IN_BOTTOM_MID, 0, BUTTON_TEXT_Y_OFFSET);
		}	
	}	
}

void lv_clear_preHeat(void)
{
	lv_obj_del(scr);
}
